...
const ChatMessages: FC<ChatMessagesProps> = ({
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      console.log('Connected to WebSocket from ChatMessages');
    });

    // Listen for channel messages
    const channelEvent = `channel:${chatId}:channel-messages`;
    const dmEvent = `direct_message:${chatId}:new-message`;
    
    const eventName = type === 'Channel' ? channelEvent : dmEvent;

    newSocket.on(eventName, (message: MessageWithUser) => {
  ...
  // Handle new message from parent callback (TextEditor)
  useEffect(() => {
    if (onNewMessage) {
      // This will be called when TextEditor sends a message
      // We can use this as a backup if socket fails
      console.log('Parent callback available for new messages');
    }
  }, [onNewMessage]);

  // Optional: Use your existing hook if needed
  useChatSocketConnection({
    ...
    updateKey: type === 'Channel'
      ? `channel:${chatId}:channel-messages:update`
      : `direct_message:${chatId}:update`,
    paramValue,
  });
