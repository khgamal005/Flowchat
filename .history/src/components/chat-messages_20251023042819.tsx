...
type ChatMessagesProps = {
  ...
  channelData?: Channel;
  onNewMessage?: (message: MessageWithUser) => void; // Add this prop
};
...
const ChatMessages: FC<ChatMessagesProps> = ({
  ...
  workspaceData,
  channelData,
  onNewMessage, // Receive callback from parent
}) => {
  ...
  useEffect(() => {
    ...
    // Listen for the specific channel event from your API
    newSocket.on(`channel:${chatId}:channel-messages`, (message) => {
  ...
  // Handle new message from parent callback
  useEffect(() => {
    if (onNewMessage) {
      // This would be called when TextEditor sends a message
      // We're already handling it via socket, so this is backup
    }
  }, [onNewMessage]);
useChatSocketConnection({
  queryKey: type === 'Channel' ? `channel:${chatId}` : `direct_message:${chatId}`,
  addKey: type === 'Channel' 
