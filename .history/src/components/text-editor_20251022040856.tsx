  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (socket && type === 'Channel' && channel) {
        // Emit to the general message channel
        socket.emit('message', {
          type: 'new_message',
        ...
        // Also emit to the specific channel event that useChatSocketConnection listens for
        socket.emit(`channel:${channel.id}:channel-messages`, result.data);
      } else if (socket && type === 'DirectMessage' && recipientId) {
        // For direct messages
        socket.emit('direct_messages:post', result.data);
      }

      // Optional: Call parent callback (not needed if using socket approach)
