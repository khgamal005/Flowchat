...
const ChatMessages: FC<ChatMessagesProps> = ({
  ...
  useEffect(() => {
    const newSocket = io({ path: '/api/web-socket/io' });
  ...
  // ✅ Handle messages from parent callback (backup to socket)
  useEffect(() => {
    if (!onNewMessage) return;
    // optional custom behavior
  }, [onNewMessage]);

  // ✅ Hook for socket data updates
  ...
  // ✅ Combine fetched + optimistic messages
  const allMessages = [
    ...optimisticMessages,
  ...
  return (
    <div ref={chatRef} className="flex-1 flex flex-col py-4 overflow-y-auto">
      ...
      {/* messages are reversed to show latest at bottom */}
