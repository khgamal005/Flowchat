...
app.prepare().then(() => {
  ...
  const io = new Server(server, {
    path: '/api/web-socket/io', // This is the base path
    cors: {
      origin: '*',
      methods: ['GET', 'POST'],
    },
  ...
  console.log('🔧 Socket.IO server configured with path:', '/api/web-socket/io');

  // 🔥 Channel Messages Namespace - matches your client URL
  ...
  // 🔥 Direct Messages Namespace - matches your client URL  
  ...
  // 🔥 Add main namespace for debugging
  io.on('connection', (socket) => {
    console.log('🟢 Client connected to MAIN namespace:', socket.id);
    
    socket.onAny((event, payload) => {
      console.log(`📡 MAIN namespace event: ${event}`);
    });
  });
  ...
  server.listen(port, () => {
    ...
    console.log(`> 🌐 Full WebSocket URLs:`);
    console.log(`>   - http://${hostname}:${port}/api/web-socket/io/api/web-socket/messages`);
    console.log(`>   - http://${hostname}:${port}/api/web-socket/io/api/web-socket/direct-messages`);
  });
});